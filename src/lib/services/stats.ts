...
export async function getExpenseStats(filter: GetStatsFilter) {
	...
	>((acc, e) => {
		...
		if (!acc[key])
			acc[key] = { name: key, color: e.category.color, total: 0, count: 0 };
	...
	for (let day = 1; day <= daysInMonth; day += 7) {
		...
		const weekEndExclusive = new Date(
			Date.UTC(year, month - 1, weekEndDay + 1),
		);
		const weekAmount = expenses
			.filter((e) => e.date >= weekStart && e.date < weekEndExclusive)
			.reduce((s, e) => s + e.amount, 0);
		const label = `${weekStart.getUTCDate()}/${weekStart.getUTCMonth() + 1}-${weekEndDay}/${month}`;
