...
const MAX_TELEGRAM_OCR_IMAGE_BYTES = 8 * 1024 * 1024;
...
export async function handleTelegramUpdate(update: TelegramUpdate) {
	...
	await safeSendTelegramMessage(
		chatId,
		"Mandame una foto de un ticket o conectá tu cuenta con /start CODIGO desde la app.",
	);
}
...
async function handleStart(
	...
	if (!code) {
		...
		await safeSendTelegramMessage(
			chatId,
			"Generá un código en la app y mandame /start CODIGO para conectar Telegram.",
		);
		return;
	}
	...
	if (!pairing || pairing.consumedAt || pairing.expiresAt < new Date()) {
		...
		await safeSendTelegramMessage(
			chatId,
			"Ese código no existe o ya expiró. Generá uno nuevo en la app.",
		);
		return;
	}
	...
	await prisma.$transaction(async (tx) => {
		...
		await tx.telegramPairingCode.update({
			where: { id: pairing.id },
			data: { consumedAt: new Date() },
		});
	});
	...
	await safeSendTelegramMessage(
		chatId,
		"Telegram conectado. Mandame fotos de tickets y las voy a dejar como drafts para revisar en Escanear.",
	);
}
...
async function handlePhoto(
	...
	if (!connection || connection.status !== "ACTIVE") {
		...
		await safeSendTelegramMessage(
			chatId,
			"Este chat no está conectado. Generá un código en la app y mandame /start CODIGO.",
		);
		return;
	}
	...
	const existing = await findTelegramReceiptDraft(
		connection.userId,
		photo.file_unique_id,
	);
	if (existing) {
		telegramLog("photo_duplicate", {
			traceId,
			chatId,
			draftId: existing.id,
		...
		await safeSendTelegramMessage(
			chatId,
			"Ese ticket ya estaba recibido. Lo tenés en drafts de Escanear.",
		);
		return;
	}
	...
	await safeSendTelegramMessage(
		chatId,
		"Recibí el ticket. Lo dejé en cola y lo voy a procesar con OCR...",
	);

	await enqueueTelegramOcr(traceId, draft.id, () =>
		processTelegramReceiptDraft(
			draft.id,
			connection.userId,
			chatId,
			message,
...
async function processTelegramReceiptDraft(
	...
	try {
		telegramLog("ocr_started", {
			traceId,
			chatId,
			draftId,
			messageId: message.message_id,
		});
		if (photo.file_size && photo.file_size > MAX_TELEGRAM_OCR_IMAGE_BYTES) {
		...
		telegramLog("telegram_file_downloaded", {
			traceId,
			chatId,
			draftId,
		...
		await updateReceiptDraft(draftId, userId, {
			...
			status: "ERROR",
			error:
				error instanceof Error ? error.message : "Error al procesar ticket",
			telegramChatId: chatId,
			telegramMessageId: message.message_id,
			telegramFileUniqueId: photo.file_unique_id,
		});
		telegramError("ocr_failed", error, {
			traceId,
			chatId,
			draftId,
			messageId: message.message_id,
		...
		await safeSendTelegramMessage(
			chatId,
			"No pude procesar ese ticket. Quedó como draft con error en Escanear.",
		);
	}
}

function enqueueTelegramOcr(
	traceId: string,
	draftId: string,
	job: () => Promise<void>,
) {
...
async function downloadTelegramFile(
	...
	if (file.file_size && file.file_size > MAX_TELEGRAM_OCR_IMAGE_BYTES) {
		throw new Error("La imagen no puede superar 8 MB");
	}

	const token = requireTelegramBotToken();
	telegramLog("telegram_file_download_started", {
		traceId,
		draftId,
	...
	if (contentLength > MAX_TELEGRAM_OCR_IMAGE_BYTES) {
		throw new Error("La imagen no puede superar 8 MB");
	}

	const buffer = Buffer.from(await res.arrayBuffer());
	if (buffer.byteLength > MAX_TELEGRAM_OCR_IMAGE_BYTES) {
		throw new Error("La imagen no puede superar 8 MB");
	}
...
async function fetchWithRetry(input: string, init?: RequestInit) {
	...
	telegramError("fetch_failed", lastError, {
		target,
...
function telegramError(
	...
	console.error(
		`[telegram] ${JSON.stringify({ event, ...details, ...errorDetails(error) })}`,
	);
}
...
function errorDetails(error: unknown) {
	...
	return {
		...
		causeHostname:
			typeof causeRecord?.hostname === "string" ? causeRecord.hostname : null,
	};
}
...
async function generateUniquePairingCode() {
	for (let attempt = 0; attempt < 5; attempt++) {
		...
		const existing = await prisma.telegramPairingCode.findUnique({
			where: { code },
		});
